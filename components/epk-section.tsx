        ...
        <motion.div className="w-full max-w-6xl mx-auto" style={{ opacity }}>
          ...
          <div className="space-y-16">
            ...
            <motion.div
              ...
              className="flex flex-row justify-center items-center gap-2 sm:gap-6 md:gap-8 lg:gap-12 overflow-x-auto sm:overflow-visible"
            >
              {artists.map((artist, index) => (
                <div key={artist.id} className="flex flex-row items-center gap-2 sm:gap-6 md:gap-8 lg:gap-12 flex-shrink-0 sm:flex-shrink">
                  <Link href={`/artists/${artist.id}`} className="block">
                    <motion.div whileHover={{ scale: 1.02 }} className="cursor-pointer transition-all p-2 sm:p-6">
                      <div className="flex flex-col items-center text-center space-y-2 sm:space-y-4">
                        <img
                          src={artist.image || "/placeholder.svg"}
                          alt={artist.name}
                          className="w-80 sm:w-100 h-80 sm:h-100 rounded-sm object-cover opacity-90"
                          style={{ objectPosition: "center top" }}
                        />
                        <div>
                          <h3 className="text-lg sm:text-xl font-light text-white/95">
                            {artist.id === "p" && (
                              <>
                                <span className="text-2xl sm:text-3xl font-black text-[#B58657]/90">P</span>
                                {artist.name.slice(1)}
                              </>
                            )}
                            {artist.id === "b" && (
                              <>
                                <span className="text-2xl sm:text-3xl font-black text-[#C44D58]/90">B</span>
                                {artist.name.slice(1)}
                              </>
                            )}
                            {artist.id === "j" && (
                              <>
                                <span className="text-2xl sm:text-3xl font-black text-[#935BAD]/90">J</span>
                                {artist.name.slice(1)}
                              </>
                            )}
                          </h3>
